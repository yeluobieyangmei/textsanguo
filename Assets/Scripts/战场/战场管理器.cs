...
public class 战场管理器 : MonoBehaviour
{
    ...
    public 战场入口管理器 战场入口管理器; // 新增：引用战场入口管理器

    //战场时间配置 - 统一时间设置中心
    [Tooltip("宣战后到战场正式开始的等待时间（秒）")]
    private float 准备时间 = 35f; // 测试时可改为10秒

    [Tooltip("战场主战时长（秒），默认1800秒=30分钟")]
    private float 主战时长 = 120f; // 测试时可改为120秒

    [Tooltip("加时赛时长（秒），默认300秒=5分钟")]
    private float 加时时长 = 300f; // 测试时可改为30秒

    [Tooltip("战场入口UI显示阈值（秒），剩余准备时间小于此值时显示UI")]
    private float UI显示阈值 = 10f;

    // 公开属性用于其他脚本访问
    public float UI显示阈值公开 => UI显示阈值;
    ...
    public void 启动王城战(国家信息库 国家, 家族信息库 家族1, 家族信息库 家族2)
    {
        ...
        // 使用新的性能优化系统：通知战场入口管理器开始倒计时
        if (战场入口管理器 != null)
        {
            战场入口管理器.开始战场准备倒计时(准备时间, UI显示阈值公开);
            // 同时启动管理器的倒计时协程来处理战场开始
    ...
    /// 启动战场主战倒计时
    /// </summary>
    private void 启动战场倒计时(战场实例 战场)
    {
        战场.当前剩余时间 = 主战时长; // 使用统一配置
        战场.倒计时进行中 = true;
        战场.是否加时阶段 = false;

        Debug.Log($"战场 {战场.战场ID} 开始主战倒计时：{主战时长}秒 ({主战时长 / 60}分钟)");

        StartCoroutine(战场时长倒计时协程(战场));
    }

    /// <summary>
    /// 战场时长倒计时协程
    /// </summary>
    private IEnumerator 战场时长倒计时协程(战场实例 战场)
    {
        while (战场.倒计时进行中 && 战场.当前剩余时间 > 0 && 战场.战场状态 == 战场状态.进行中)
        {
            yield return new WaitForSeconds(1f);
            战场.当前剩余时间 -= 1f;
            ...
            if (战场.当前剩余时间 % 60 == 0)
            {
                Debug.Log($"战场 {战场.战场ID} 倒计时：还剩 {战场.当前剩余时间 / 60} 分钟");
            }
        }

        // 时间到，处理结算
        if (战场.倒计时进行中 && 战场.战场状态 == 战场状态.进行中)
        {
            处理战场倒计时结束(战场);
        }
    }
    ...
    /// 处理战场倒计时结束
    /// </summary>
    private void 处理战场倒计时结束(战场实例 战场)
    {
        if (战场 == null) return;

        Debug.Log("战场倒计时结束，开始结算...");

        int 家族1积分 = 战场.家族1积分;
        int 家族2积分 = 战场.家族2积分;

        Debug.Log($"当前积分 - {战场.家族1.家族名字}: {家族1积分}, {战场.家族2.家族名字}: {家族2积分}");
        ...
        else
        {
            ...
            {
                // 加时后仍然平分，宣布平局
                Debug.Log("加时结束，双方平分，宣布平局");
                结束战场(战场, null); // null表示平局
            }
        }
    }
    ...
    /// 开始加时阶段
    /// </summary>
    private void 开始加时阶段(战场实例 战场)
    {
        战场.是否加时阶段 = true;
        战场.当前剩余时间 = 加时时长; // 使用统一配置

        Debug.Log($"双方积分相同，进入加时阶段：{加时时长}秒 ({加时时长 / 60}分钟)");
    ...
    private void 结束战场(战场实例 战场, 家族信息库 胜利家族)
    {
        ...
        if (胜利家族 != null)
        {
            ...
            Debug.Log($"{全局变量.所有玩家数据表[胜利家族.族长ID].姓名} 登顶王位！，{战场.所属国家.国号}的执政家族修改为：{战场.所属国家.执政家族.家族名字}");
        }

        // 停止战场协程
        StopCoroutine(战场逻辑协程(战场));

        // 清理战场数据（但不清空执政家族，只清空宣战状态）
        战场.所属国家.宣战家族1 = null;
        战场.所属国家.宣战家族2 = null;

        战场.所属国家.宣战家族1 = 战场.胜利家族;
        // 清空参战玩家列表
        战场.参战玩家列表.Clear();
