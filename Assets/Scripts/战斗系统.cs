...
public static class 战斗系统
{
    ...
    */
    ...
    private static int 计算伤害(int 攻击力, int 防御力, float 暴击率 = 0)
    {
        // 使用double类型避免数值溢出，并优化公式
        double 攻击力_d = (double)攻击力;
        double 防御力_d = (double)防御力;
        
        // 新公式：基础伤害 = 攻击力 * (1 - 防御力 / (攻击力 + 防御力))
        // 这个公式更合理，防御力永远不会完全抵消伤害
        double 防御系数 = 防御力_d / (攻击力_d + 防御力_d);
        double 基础伤害 = 攻击力_d * (1.0 - 防御系数);
        
        // 确保最小伤害为1
        int 最终伤害 = Mathf.Max(1, (int)基础伤害);
        
        // 暴击判定
        if (Random.value < (基础暴击率 + 暴击率))
        {
            最终伤害 = (int)(最终伤害 * 暴击伤害倍率);
            战斗日志管理器.实例.添加日志("暴击！\n");
        }
        
        Debug.Log($"伤害计算: 攻击力={攻击力}, 防御力={防御力}, 防御系数={防御系数:F3}, 基础伤害={{基础伤害:F0}}, 最终伤害={最终伤害}");
